...
- 指导你把抖音 Cookie 保存到本地 `cookies.txt`
- 下载视频、抽音频、转写，并由你当前正在使用的 AI 读取转写稿后总结、写入 Markdown
- 遇到 Windows 引号、相对路径、DouK 下载记录等问题时按内置恢复流程处理

人类只需要负责：提供视频 URL、本地 Cookie 文件路径、同意安装/读取浏览器/关闭浏览器等敏感操作。不要把 Cookie 发到聊天里。

需要用户同意时，执行 AI 应使用 agent 的 ask/approval 能力发起确认；不要要求用户在聊天里输入 `yes`、`y`、`1` 之类的确认文本。
...
- 总结：默认由你当前正在使用的 AI 读取转写稿并总结；本地 `Ollama` 只是可选方案
...
示例使用 `yt-dlp` 下载、`ffmpeg` 抽音频、`faster-whisper` 转写。本示例不传 `--summarize-command`，生成转写稿后由当前 AI 总结：

```bash
python -m video2md "https://www.douyin.com/video/xxx" \
  --title "抖音视频笔记" \
  --download-command 'yt-dlp -o "{video}" "{url}"' \
  --transcribe-command 'python scripts/transcribe_faster_whisper.py "{audio}" "{transcript}" --model small --device cpu --compute-type int8' \
  --output-dir ./knowledge \
...
推荐路线是 `DouK-Downloader + cookies.txt + video2md`。抖音网页下载基本需要登录态，脚本要求先提供本地 `cookies.txt`，避免无效重试浪费时间。这条路线已经实测跑通，比直接用 `yt-dlp` 更适合中国抖音。
...
没有本地 LLM 不影响流程：先生成转写稿，再让当前 AI 直接总结。

完成后，AI 应根据内容摘要自动生成一个简短标题，把 `knowledge/<id>.md` 重命名为类似 `knowledge/<内容标题>.md` 的文件。不要把最终结果只停留在视频 id 文件名。

AI 还应统一报告过程文件位置，包括：

- 项目目录：`<workspace>`
- 工具缓存：`<tool-cache>`
- DouK：`<tool-cache>/DouK-Downloader`
- DouK 依赖：`<tool-cache>/douk-deps`
- ASR 依赖：`<tool-cache>/asr-deps`
- 模型缓存：`<tool-cache>/hf-cache`
- Cookie 文件：`<workspace>/cookies.txt`，只报告路径，不打印内容
- 本次运行目录：`<workspace>/runs/<id>`
- 视频、音频、转写稿、最终 Markdown 笔记的完整路径
...
- 如果遇到 `cublas64_12.dll` 或 CUDA 相关错误，说明转录尝试走 GPU。0 基础流程必须使用 `--device cpu --compute-type int8`。
...
脚本层面要求提供本地 `cookies.txt`。不要做无 Cookie 尝试。
...
export VIDEO2MD_TRANSCRIBE_COMMAND='python scripts/transcribe_faster_whisper.py "{audio}" "{transcript}" --model small --device cpu --compute-type int8'
export VIDEO2MD_SUMMARIZE_COMMAND='python scripts/summarize_ollama.py "{transcript}" "{summary}" --model qwen2.5:7b'
```

上面的总结环境变量只适合你明确要用本地 Ollama 的情况。面向 0 基础用户时，优先让当前 AI 读取转写稿并总结。
...
如果不传 `--summarize-command`，程序生成的笔记会先包含转写稿；随后让当前 AI 读取转写稿并补充摘要与要点。
